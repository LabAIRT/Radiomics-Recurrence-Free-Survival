...
# randstate = 0
writefilename = 'Validation_Prediction_CliFea_Expectation_spearman0.8_AnalCancer_StratiTrainValTest_'
...
cli_data = extract_cli_features("Clinical_feature_Standard_AnalCancer.xlsx", "Sheet1")

# Checking for null values
features = cli_data.columns[1:-1]
...
endpoint_data['Duration'][endpoint_data['Duration']>1200]=1200
...
for randstate in range(5, 15):
    ...
    for index in range(0, folds):
        ...
        cols_standardize = ['CD4 counts', 'T', 'N', 'Age at Dx']
        cols_categorical = ['HIV status', 'Transplant recipient', 'Gender (1Male 0Female)',
                            'smoking status (0never 1former 2current)',
                            'M', 'Inguinal node', 'Internal iliac', 'External iliac', 'Mesorectal']
